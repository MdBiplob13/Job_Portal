...
export default function ProfessionalBids() {
  ...
  const { proposedBids = [], loading } =
    useGetAllProposedBidsForProfessional(professionalId);
  ...
  const stats = useMemo(() => {
    ...
    list.forEach((proposal) => {
      ...
      if (proposalStatus !== "rejected" && bidStatus !== "completed")
        s.active++;
    });
  ...
  const filteredProposals = useMemo(() => {
    ...
    if (activeTab === "finished") {
      list = list.filter(
        (proposal) => proposal.bidId?.status?.toLowerCase() === "completed"
      );
  ...
  const getActionButtons = (proposal) => {
    ...
    if (proposalStatus === "pending" && bidStatus === "completed") {
      return (
        <Link
          ...
          <div className="flex justify-between items-start mb-3">
            <div className="flex-1">
              ...
              <p className="text-gray-600 mt-1">
                {bid.companyName || "No company name"}
              </p>
            </div>

            <div className="flex flex-col gap-2 items-end">
              <span
                className={`px-3 py-1 rounded-full text-sm font-semibold border ${getProposalStatusColor(
                  proposalStatus
                )}`}
              >
                {capitalise(bidStatus)} Proposal
              </span>
      ...
      >
        ...
        <div className="p-6 space-y-8 overflow-y-auto h-[calc(100vh-140px)]">
          ...
          <div>
            ...
            <div className="space-y-3">
              {["Any", "1-2 weeks", "1 month", "1-3 months", "3+ months"].map(
                (duration) => (
                  <label
                    key={duration}
                    className="flex items-center cursor-pointer group"
          ...
          {searchQuery && (
            <div className="mt-4 flex items-center justify-between">
              <p className="text-sm text-gray-600">
                ...
                {filteredProposals.length !== 1 ? "s" : ""} for "{searchQuery}"
              </p>
              <button
