...
export default function EmployerBids() {
  ...
  // using ALL BIDS from admin hook
  const {
    allBids,
    allBidsLoading,
    setAllBidsRefresh,
  ...
  const stats = useMemo(() => {
    ...
    if (!Array.isArray(allBids)) return s;

    s.total = allBids.length;

    allBids.forEach((b) => {
      const st = (b.status || "").toLowerCase();
      if (st === "pending") s.pending++;
      if (st === "accepted") s.accepted++;
      if (st === "in progress") s.inProgress++;
  ...
  const getActionButtons = (b) => {
    ...
    switch (status) {
      ...
      case "accepted":
        return (
          <>
            <button className="cursor-pointer flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg hover:border-blue-500 hover:text-blue-500 transition-colors">
              <FiMessageCircle className="text-sm" /> Contact
            </button>

            <button className="cursor-pointer flex items-center gap-2 px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors">
              <FiAward className="text-sm" /> Manage
            </button>
          </>
        );
      ...
      case "in progress":
        return (
          <button className="cursor-pointer flex items-center gap-2 px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 transition-colors w-full justify-center">
            <FiClock className="text-sm" /> In Progress
          </button>
        );

      case "completed":
        return (
          <button className="cursor-pointer flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-800 rounded-lg w-full justify-center">
      ...
      case "rejected":
        return (
          <button className="cursor-pointer flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg w-full justify-center">
            <FiEye className="text-sm" /> View
          </button>
        );
  ...
  const FilterDrawer = () => (
    <>
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 border-t border-gray-200 bg-white flex gap-3">
          <button
            onClick={handleClearFilters}
            className="cursor-pointer flex-1 py-3 border-2 border-gray-300 text-gray-700 rounded-xl font-semibold hover:border-gray-400"
          >
            Clear All
          </button>
          ...
          <button
            onClick={() => setIsFilterOpen(false)}
            className="cursor-pointer flex-1 py-3 bg-blue-500 text-white rounded-xl font-semibold"
          >
            Apply
          </button>
        </div>
    ...
    <div className="min-h-screen bg-linear-to-br from-gray-50 to-gray-100 p-6">
      ...
      <div className="mb-8">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
          <div>
            <h1 className="text-3xl font-bold text-gray-800">Admin page Bids & Tenders</h1>
          ...
          <Link
            ...
            className="cursor-pointer bg-blue-500 text-white px-6 py-3 rounded-xl font-semibold hover:bg-blue-600 transition-colors flex items-center gap-2 w-fit"
          >
            <FiPlus className="text-lg" /> Post New Bid
          </Link>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <div className="flex flex-col md:flex-row gap-4 items-center">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="cursor-pointer w-full pl-12 pr-4 py-3 bg-gray-50 rounded-xl border border-gray-200 focus:ring-2 focus:ring-blue-500 outline-none transition-all"
              />
            </div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="cursor-pointer px-6 py-3 border-2 border-gray-300 rounded-xl font-semibold text-gray-700 hover:border-blue-500 hover:text-blue-500 transition-colors flex items-center gap-2"
              >
              ...
              <button
                onClick={() =>
                  setAllBidsRefresh((prev) => (prev || 1) + 1)
                }
                className="cursor-pointer px-6 py-3 bg-blue-500 text-white rounded-xl font-semibold hover:bg-blue-600"
              >
                Refresh
              </button>
            </div>
          ...
          {filters.status !== "all" || filters.urgency !== "all" ? (
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <button
                onClick={handleClearFilters}
                className="cursor-pointer text-gray-500 hover:text-gray-700 text-sm font-medium underline"
              >
                Clear all
              </button>
            </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        {allBidsLoading ? (
          ...
          <div className="col-span-2 bg-white rounded-2xl p-12 text-center border border-gray-200">
            ...
            <Link
              href={"/pages/dashboard/employer/post/bids"}
              className="cursor-pointer bg-blue-500 text-white px-6 py-2 rounded-lg font-semibold hover:bg-blue-600 transition-colors inline-block"
            >
              Post New Bid
            </Link>
          </div>
      ...
      {filteredBids.length > 0 && (
        <div className="mt-8 text-center">
          <button className="cursor-pointer bg-white border border-gray-300 text-gray-700 px-8 py-3 rounded-xl font-semibold hover:border-gray-400">
            Load More
          </button>
        </div>
      )}
